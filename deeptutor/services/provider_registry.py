...
PROVIDER_ALIASES = {
    ...
    "volcengineCodingPlan": "volcengine_coding_plan",
    "volcengineagentplan": "volcengine_agent_plan",
    "volcengineAgentPlan": "volcengine_agent_plan",
...
PROVIDERS: tuple[ProviderSpec, ...] = (
    ...
    # Agent Plan keys are rejected by the plain /api/v3 gateway (auth error),
    # so this must stay ahead of the "volces" keyword match below.
    ProviderSpec(
        name="volcengine_agent_plan",
        keywords=("volcengine-agent-plan", "agent-plan"),
        env_key="OPENAI_API_KEY",
        display_name="VolcEngine Agent Plan",
        backend="openai_compat",
        is_gateway=True,
        detect_by_base_keyword="/api/plan/v3",
...
def find_gateway(
    ...
    for spec in PROVIDERS:
        _probe_key = api_key[0] if isinstance(api_key, list) and api_key else api_key
        if spec.detect_by_key_prefix and _probe_key and _probe_key.startswith(spec.detect_by_key_prefix):
            return spec
