...
class TurnExecutor:
    ...
    async def _run_turn(self, execution: _TurnExecution) -> None:
        ...
        try:
            ...
            # ── 悦学方案B：⑧掌握度门禁软校验（gate_skill 迁移自旧 routers/chat.py）──
            # 仅当会话 settings 显式启用 gate_skill 时生效（声明式，不污染其它对话）。
            # 违规时不重跑整轮，直接把补报分消息追加到 result。宿主端记账
            # （example_skipped/max_hint_level/low_confidence）跨轮存于会话
            # settings.gate_state，掌握度不依赖模型记忆。
            try:
                # ⚠️ 挂起（1.6.3 跟版）：v2 session store 已无 per-session
                ...
                _gate_session = await self.store.get_session(session_id)
                _gate_settings = (_gate_session or {}).get("settings") or {}
                if _gate_settings.get("gate_skill"):
                    _gate_state = _gate_settings.get("gate_state") or init_gate_state()
                    update_gate_state(_gate_state, effective_user_message)
                    _remedy_msg, _ = check_and_remedy(assistant_content, _gate_state)
                    if _remedy_msg:
                        assistant_content = (
                    ...
                    if any(
                        _gate_state.get(k)
                        for k in ("example_skipped", "max_hint_level", "low_confidence")
                    ):
                        await self.store.update_session_settings(
                            session_id, {"gate_state": _gate_state}
                        )
            except Exception:
                logger.exception("gate_skill check failed for session %s", session_id)
