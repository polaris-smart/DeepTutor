...
import { Check, ClipboardList, EyeOff } from "lucide-react";
...
export interface ModuleTestBlockProps {
  block: Block;
}

export default function ModuleTestBlock({ block }: ModuleTestBlockProps) {
  const { t } = useTranslation();
  const questions =
    (block.payload?.questions as TestQuestion[] | undefined) || [];
  const moduleNames =
    (block.payload?.module_names as string[] | undefined) || [];
  // Same reveal-per-question rule as the practice block: one answer open at a
  // time, so finishing the paper stays an actual attempt.
    ...
    <div className="rounded-2xl border border-[var(--primary)]/30 bg-gradient-to-br from-[var(--primary)]/5 to-transparent p-4 shadow-sm">
      <div className="mb-3 flex items-center gap-2">
        ...
        <span className="ml-auto text-xs text-[var(--muted-foreground)]">
          {moduleNames.length > 0 ? moduleNames.join(" · ") : `${questions.length}`}
        </span>
      </div>
              ...
              {Object.keys(options).length > 0 && (
                <div className="mt-2 grid gap-1.5 sm:grid-cols-2">
                  {Object.entries(options).map(([label, body]) => {
                    ...
                    const isAnswer = isOpen && question.answer === label;
                    return (
                      <button
                        key={label}
                        onClick={() =>
